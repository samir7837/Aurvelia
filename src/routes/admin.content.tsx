...
function AdminContent() {
  ...
  const { data: story } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiFetch('/api/site_content?key=our_story', { headers: token ? { Authorization: `Bearer ${token}` } : {} });
  ...
  const saveStory = useMutation({
    mutationFn: async () => {
      const token = localStorage.getItem('aurvelia_token');
      const res = await apiFetch('/api/site_content/our_story', { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ value: { heading, body } }) });
  ...
  const addFaq = useMutation({
    mutationFn: async () => {
      const token = localStorage.getItem('aurvelia_token');
      const res = await apiFetch('/api/faqs', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ question: 'New question', answer: 'Answer here', category: 'General', sort_order: (faqs.length || 0) + 1 }) });
  ...
  const saveFaq = useMutation({
    mutationFn: async (f: Faq) => {
      const token = localStorage.getItem('aurvelia_token');
      const res = await apiFetch(`/api/faqs/${f.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ question: f.question, answer: f.answer, category: f.category, is_active: f.is_active }) });
  ...
  const deleteFaq = useMutation({
    mutationFn: async (id: string) => {
      const token = localStorage.getItem('aurvelia_token');
      const res = await apiFetch(`/api/faqs/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
